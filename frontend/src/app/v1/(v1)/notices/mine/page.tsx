...
import { getMyNoticesHandler } from "@/core/handlers/notice/getNotices.handler";
...
export default function MyNoticesPage() {
  ...
  const { data, isLoading } = useQuery<responseNoticeSchemaType[]>({
    queryKey: ["my-notices"],
    queryFn: async () =>
      await getMyNoticesHandler(profile?.id ?? "", profile?.access ?? ""),
  });
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Meus Avisos");
    XLSX.writeFile(workbook, "meus-avisos.xlsx");
  };
  ...
  return (
    <div className="w-full h-full flex flex-col items-stretch justify-between p-2">
      ...
      <div className="flex-grow custom-scrollbar overflow-y-auto space-y-8 p-2">
        <header className="w-full flex flex-row gap-2  items-center justify-between">
          {/* Greet Message */}
        ...
        <main>
          ...
          <DataTable
            data={filterTime(
              filterDate(
                filterSearchBar(data ?? [], debouncedSearchValue),
                searchDate,
              ),
              {
